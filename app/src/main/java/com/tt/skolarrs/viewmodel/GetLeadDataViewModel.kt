...
import com.tt.skolarrs.repoistory.GetLeadDataRepo
import com.tt.skolarrs.view.activity.LoginActivity

class GetLeadDataViewModel : ViewModel() {
    private val repository = GetLeadDataRepo()
    ...
    suspend fun getLeadDataByNumber(token: String, number:String, context: Context?) {

        try {

            val  deviceId = Settings.Secure.getString(context?.contentResolver, Settings.Secure.ANDROID_ID)
            val response = repository.getLeadByPhone(token,deviceId, number )
